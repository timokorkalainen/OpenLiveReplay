...
- **Feedback:** Full status surface — state-aware icons plus live text
  ...
  touch strip.
- **Actions:** Transport set only — the same nine actions the MIDI system
  dispatches (record toggle, play/pause, go live, capture, frame step ±,
  rewind 5×, forward 5×, jog).
...
4. **Build plumbing**
   - `ios_build/build_streamdeck_bridge.sh`: xcodebuild archive → xcframework
     (same pattern as `build_ffmpeg_ios_srt.sh`)
...
|---|---|---|
| MK.2 (15), XL (32) | REC (red + elapsed when armed), Play/Pause, Go Live, Capture, Step ←, Step →, Rew 5×, Fwd 5×, Timecode display, Speed display | — |
...
- Swift diffs incoming state into granular per-key observable fields, so
  ...
  images over USB. Static icons are prerendered assets; only text overlays
  vary.
