...
static QByteArray pcm(int bytes) {
...
void TestAudioFrameQueue::releasesOnlyWithinLead() {
    ...
    QVERIFY(q.releaseDue(1000, 200, f));
    QCOMPARE(f.ptsMs, int64_t(1000));
    QVERIFY(q.releaseDue(1000, 200, f));
    QCOMPARE(f.ptsMs, int64_t(1100));
    QVERIFY(!q.releaseDue(1000, 200, f)); // 1400 > 1200, not due
    // playhead advances; 1400 becomes due.
    QVERIFY(q.releaseDue(1300, 200, f));
    QCOMPARE(f.ptsMs, int64_t(1400));
}
...
void TestAudioFrameQueue::releaseIsFifo() {
    ...
    AudioFrameQueue::Frame f;
    QVERIFY(q.releaseDue(10000, 0, f));
    QCOMPARE(f.ptsMs, int64_t(100));
    QVERIFY(q.releaseDue(10000, 0, f));
    QCOMPARE(f.ptsMs, int64_t(200));
}

void TestAudioFrameQueue::dropOlderThanBoundsMemory() {
    AudioFrameQueue q;
    for (int64_t p = 0; p <= 1000; p += 100)
        q.enqueue(p, pcm(4).constData(), 4);
    q.dropOlderThan(900, 200); // keep pts >= 700
    AudioFrameQueue::Frame f;
    QVERIFY(q.releaseDue(10000, 0, f));
    QCOMPARE(f.ptsMs, int64_t(700));
}
