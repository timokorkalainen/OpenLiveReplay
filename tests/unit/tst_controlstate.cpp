...
public:
    ...
    TransportState transportState() const override {
        return {1200, 1200, 1500, QStringLiteral("00:00:01:06"), true, 1.0, 30, true, 1000};
    }
    QVector<SourceState> sourceStates() const override {
        ...
        s.trimOffsetMs = 33;
        s.metadata.append(QVariantMap{{QStringLiteral("name"), QStringLiteral("driver")},
                                      {QStringLiteral("value"), QStringLiteral("A. Smith")}});
        return {s};
    }
    ...
    SettingsState settingsState() const override {
        return {QStringLiteral("match"),
                ...
                40,
                true,
                QVariantList{QVariantMap{{QStringLiteral("name"), QStringLiteral("driver")}}}};
    }
    MidiState midiState() const override {
        return {QStringList{QStringLiteral("X-Touch")}, 0, QStringLiteral("X-Touch"), true, -1, 0};
    }
    StreamDeckState streamDeckState() const override {
        return {true, true, QStringLiteral("Deck"), QStringLiteral("Plus"), 8, 4, -1};
    }
    ScreensState screensState() const override {
        return {true, 1,
                QVariantList{QVariantMap{{QStringLiteral("index"), 0},
                                         {QStringLiteral("label"), QStringLiteral("Display")}}}};
    }
    ImportState importState() const override {
        return {QStringLiteral("http://settings"), QStringLiteral("http://telemetry"), false,
                QString(), QVariantMap{}};
    }
...
void TestControlState::buildsSnapshotWithExpectedTopLevelObjects() {
    ...
    QCOMPARE(state.value(QStringLiteral("recording"))
                 .toObject()
                 .value(QStringLiteral("active"))
                 .toBool(),
             true);
    QCOMPARE(state.value(QStringLiteral("transport"))
                 .toObject()
                 .value(QStringLiteral("timecode"))
                 .toString(),
             QStringLiteral("00:00:01:06"));
    QCOMPARE(state.value(QStringLiteral("sources"))
                 .toArray()
                 .first()
                 .toObject()
    ...
    QCOMPARE(state.value(QStringLiteral("settings"))
                 .toObject()
                 .value(QStringLiteral("audioOutputLatencyMs"))
                 .toInt(),
             40);
    QCOMPARE(state.value(QStringLiteral("telemetry"))
                 .toObject()
...
void TestControlState::buildsPathPatch() {
    ...
    QCOMPARE(
        msg.value(QStringLiteral("value")).toObject().value(QStringLiteral("playing")).toBool(),
        true);
}
