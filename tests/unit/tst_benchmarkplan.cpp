...
void TestBenchmarkPlan::sustainPredicate() {
    RampStepResult r;
    r.concurrency = 4;
    r.framesRequired = 600;
    r.framesProcessed = 600;
    r.budgetMet = true;
    QVERIFY(rampStepSustained(r));
    r.framesProcessed = 570;
    QVERIFY(rampStepSustained(r)); // exactly 0.95
    r.framesProcessed = 560;
    QVERIFY(!rampStepSustained(r)); // below 0.95
    r.framesProcessed = 600;
    r.budgetMet = false;
    QVERIFY(!rampStepSustained(r));
}
...
void TestBenchmarkPlan::headroomPredicate() {
    ...
    r.framesRequired = 600;
    r.budgetMet = true;
    r.framesProcessed = 720;
    QVERIFY(rampStepHasHeadroom(r)); // exactly 1.2
    r.framesProcessed = 719;
    QVERIFY(!rampStepHasHeadroom(r));
}
...
void TestBenchmarkPlan::safeAndCeilingFromSteps() {
    ...
    auto mk = [](int n, int req, int got) {
        RampStepResult r;
        r.concurrency = n;
        r.framesRequired = req;
        r.framesProcessed = got;
        r.budgetMet = true;
        return r;
    };
    QVector<RampStepResult> steps{
        mk(1, 150, 300),    mk(2, 300, 600),
        mk(4, 600, 1200),   mk(8, 1200, 1260), // sustained (>=0.95*1200=1140) but < headroom (1440)
