...
#
# Usage: run_srt_hevc_smoke.sh <record_harness_exe> [srt_port]
set -uo pipefail
...
if printf '%s\n' "$ENCODERS" | grep -q ' libx265 '; then
    ...
    for encoder in hevc_mf hevc_nvenc hevc_qsv hevc_amf hevc_videotoolbox; do
        if printf '%s\n' "$ENCODERS" | grep -q " ${encoder} "; then
            SELECTED_HEVC_ENCODER="$encoder"
            break
        fi
    done
    if [ -z "$SELECTED_HEVC_ENCODER" ]; then
        SELECTED_HEVC_ENCODER="$(printf '%s\n' "$ENCODERS" \
            | awk '/ hevc_/ { for (i = 1; i <= NF; ++i) if ($i ~ /^hevc_/) { print $i; exit } }')"
    fi
    [ -n "$SELECTED_HEVC_ENCODER" ] || { echo "SKIP: no HEVC encoder available in ffmpeg"; exit 0; }
    VCODEC_ARGS=(-c:v "$SELECTED_HEVC_ENCODER" -g 30 -pix_fmt yuv420p -b:v 4M)
fi
...
OUT="$("$HARNESS" --url "$URL" --name olr_srt_hevc_smoke --outdir "$WORKDIR" \
