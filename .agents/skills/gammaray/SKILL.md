...
**First, get a launchable, current build.** Build the app per `AGENTS.md` into a
...
| `Library not loaded: @rpath/librtmidi.7.dylib` → `Injector error: Process crashed` | The app **bundle is stale** (library paths baked at an old checkout path), not a GammaRay fault. Use a build configured at the current repo path, or rebuild per `AGENTS.md`. |
