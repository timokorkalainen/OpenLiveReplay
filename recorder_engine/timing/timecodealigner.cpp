...
TimecodeAligner::TimecodeAligner(int nominalFps) : m_nominalFps(nominalFps > 0 ? nominalFps : 30) {}
...
void TimecodeAligner::observe(int sourceIndex, int64_t sourceTimecode100ns,
    ...
    if (sourceTimecode100ns < 0) return;

    Anchor& anchor = m_anchors[sourceIndex];
    if (anchor.set) return; // keep the FIRST observation as the anchor
...
int64_t TimecodeAligner::toSessionFrameIndex(int sourceIndex, int64_t mediaTimecode100ns) const {
    if (sourceIndex < 0 || sourceIndex >= kMaxSources) return -1;
    const Anchor& anchor = m_anchors[sourceIndex];
    if (!anchor.set) return -1;
...
int64_t TimecodeAligner::frameOffset(int sourceIndexA, int sourceIndexB) const {
    if (sourceIndexA < 0 || sourceIndexA >= kMaxSources) return 0;
    if (sourceIndexB < 0 || sourceIndexB >= kMaxSources) return 0;
...
bool TimecodeAligner::sourcesAligned(int sourceIndexA, int sourceIndexB,
    ...
    if (!hasTimecode(sourceIndexA) || !hasTimecode(sourceIndexB)) return false;
