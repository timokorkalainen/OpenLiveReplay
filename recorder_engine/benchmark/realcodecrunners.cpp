                         ...
                         const std::vector<ThreadResult>& results)
{
    ...
    for (const auto& t : results) {
        ...
        if (t.overran) r.budgetMet = false;
...
RampStepResult Mpeg2CodecRunner::runStep(int concurrency, const BenchmarkConfig& cfg)
{
    ...
    auto threadFn = [&](int idx) {
        ...
        if (!decoder) { avcodec_free_context(&encCtx); return; }
        AVCodecContext* decCtx = avcodec_alloc_context3(decoder);
        if (!decCtx) { avcodec_free_context(&encCtx); return; }
        ...
        while (wall.elapsed() < cfg.durationMsPerStep) {
            ...
            if (totalMs > budgetMs) res.overran = true;
...
RampStepResult H264CodecRunner::runStep(int concurrency, const BenchmarkConfig& cfg)
{
    ...
    auto threadFn = [&](int idx) {
        ...
        if (!enc) return; // should not happen: available() returned true
        ...
        if (!parseAvcc(avcc, &sps, &pps)) return;
