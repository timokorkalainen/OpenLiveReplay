...
QString nativeSrtConnectFailureMessage(int srtErrorCode, const QString& lastError, int rejectReason,
                                       const QString& rejectReasonText) {
    if (rejectReason != kSrtRejectUnknown) {
        const QString rejectSummary =
            rejectReasonText.isEmpty()
